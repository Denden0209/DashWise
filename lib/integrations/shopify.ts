...
async function shopifyFetch(shopDomain:string, accessToken:string, endpoint:string, params:Record<string,string>={}): Promise<unknown> {
  ...
  const res = await fetch(url.toString(), { headers:{ "X-Shopify-Access-Token":accessToken, "Content-Type":"application/json" } });
...
export function buildShopifyAnalytics(orders:ShopifyOrder[], products:ShopifyProduct[], daysBack:number): ShopifyAnalytics {
  ...
  const pRev: Record<string,{revenue:number;units:number}> = {};
  paid.forEach(o=>o.line_items?.forEach(item=>{
    if(!pRev[item.title])pRev[item.title]={revenue:0,units:0};
    pRev[item.title].revenue+=parseFloat(item.price)*item.quantity;
    pRev[item.title].units+=item.quantity;
  }));
  const topProducts = Object.entries(pRev).map(([title,d])=>({title,...d})).sort((a,b)=>b.revenue-a.revenue).slice(0,10);

  const byDay: Record<string,{revenue:number;orders:number}> = {};
  paid.forEach(o=>{
    const date=o.created_at.split("T")[0];
    if(!byDay[date])byDay[date]={revenue:0,orders:0};
    byDay[date].revenue+=parseFloat(o.total_price);
    byDay[date].orders+=1;
  });
