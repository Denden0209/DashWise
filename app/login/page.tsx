    ...
    <div style={{ minHeight:"100vh", background:"#f5f5f7", display:"flex", alignItems:"center", justifyContent:"center", padding:24 }}>
      <div style={{ width:"100%", maxWidth:400 }}>
        <div style={{ textAlign:"center", marginBottom:36 }}>
          ...
          <h1 style={{ fontSize:22, fontWeight:600, color:"#1d1d1f", marginBottom:4 }}>Welcome back</h1>
          <p style={{ fontSize:14, color:"#86868b" }}>Sign in to your account</p>
        </div>
        ...
        <div style={{ background:"#fff", border:"1px solid #e5e5ea", borderRadius:20, padding:28, boxShadow:"0 4px 16px rgba(0,0,0,0.06)" }}>
          ...
          <form onSubmit={handleEmail} style={{ display:"flex", flexDirection:"column", gap:14 }}>
            <div>
              <label style={{ display:"block", fontSize:12, fontWeight:500, color:"#515154", marginBottom:6 }}>Email</label>
              <input type="email" required value={email} onChange={e=>setEmail(e.target.value)} placeholder="you@company.com" style={inp}/>
            </div>
            <div>
              <label style={{ display:"block", fontSize:12, fontWeight:500, color:"#515154", marginBottom:6 }}>Password</label>
              <div style={{ position:"relative" }}>
                <input type={showPass?"text":"password"} required value={password} onChange={e=>setPassword(e.target.value)} placeholder="Your password" style={{ ...inp, paddingRight:52 }}/>
                <button type="button" onClick={()=>setShowPass(!showPass)} style={{ position:"absolute", right:12, top:"50%", transform:"translateY(-50%)", background:"none", border:"none", color:"#86868b", fontSize:12, cursor:"pointer", fontFamily:"inherit" }}>
                  {showPass?"Hide":"Show"}
                </button>
              </div>
            </div>
        ...
        <p style={{ textAlign:"center", marginTop:20, fontSize:14, color:"#86868b" }}>
          ...
          <Link href="/signup" style={{ color:"#0071e3", fontWeight:500 }}>Sign up free</Link>
        </p>
      </div>
    </div>
  );
